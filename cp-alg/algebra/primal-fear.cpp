...
char check(ul n) {
    // print("n is ", n);
    if (nums[n] == 0) {
        ul next_num = get_next_num (n);
        ul floor_num = get_floor(n);
        // print(n, next_num, floor_num);
        if (next_num < floor_num || nums[n] == 1){
            return 1;
        } else {
            nums[n] = check ( next_num );
        }
    }
    return nums[n];
}
...
int main() {
	ul n, x, r, c;
    seive();
    nums[2] = nums[3] = nums[5] = nums[7] = 2; // used in find_primes
    find_primes();
    cin >> n;
    ...
    while (n--) {
        cin >> x;
        it = upper_bound(primes.begin(), primes.end(), x);
        print(it - primes.begin());

    }
    // for (ul i = 0; i < primes.size(); i++) {
    //     print(primes[i]);
    // }
    return 0;
