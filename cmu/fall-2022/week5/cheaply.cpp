...
int main() {
    ...
    vector<int> v;
    cin >> n >> x;
    v.push_back(-1); // dummy value for indexing purposes
    while (n--) {
        cin >> r;
        v.push_back(r);
    }
    ...
    int total = v[1];
    int index;
    pair<int,int> p = make_pair(total,1);
    pair<int,int> out;
    pq.push(p);
    ...
    while (count != x) {
        ...
        index = out.s;
        if (index + 1 < v.size()) {
            p = make_pair(total - v[index] + v[index + 1], index+1);
            pq.push(p);
            p = make_pair(total + v[index + 1], index+1);
            pq.push(p);
        }
        count++; 
    }
